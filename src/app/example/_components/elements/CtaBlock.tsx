...
export function CtaBlock({ title, description, link, image_url, target_url }: Props) {
  const href = target_url || link || '#'
  ...
  return (
    <section className="rounded-lg border border-neutral-200 bg-gradient-to-r from-blue-50 to-white p-6 space-y-3">
      ...
      <a href={href} className="inline-block rounded-md bg-blue-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-700">{title || 'Learn more'}</a>
    </section>
  )
}
