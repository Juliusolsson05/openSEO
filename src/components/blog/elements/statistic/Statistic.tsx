...
export function Statistic({ content, blogId, elementId, onContentUpdated, onElementDeleted, onElementAdded }: ElementComponentProps) {
  ...
  return (
    <BaseElement content={content} blogId={blogId} elementId={elementId} allowEdit={false} onContentUpdated={onContentUpdated} onElementDeleted={onElementDeleted} onElementAdded={onElementAdded}>
      <div className="space-y-2 rounded-lg bg-secondary p-[30px]">
        {editing ? (
          ...
          >
            <h3 className="mb-4 text-center text-2xl font-semibold" dangerouslySetInnerHTML={{ __html: renderMarkdownInline(view.title ?? '') }} />
            ...
            <p className="text-center text-[1.125rem] font-light" dangerouslySetInnerHTML={{ __html: renderMarkdownInline(view.label ?? view.description ?? '') }} />
            {view.source ? <p className="mt-2 text-center text-xs text-muted-foreground" dangerouslySetInnerHTML={{ __html: renderMarkdownInline(view.source) }} /> : null}
          </div>
        )}
      </div>
    </BaseElement>
